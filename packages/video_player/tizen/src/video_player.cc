...
flutter::EncodableList VideoPlayer::GetAudioTracks() {
  ...
  int track_count = 0;
  int ret =
      player_get_track_count(player_, PLAYER_STREAM_TYPE_AUDIO, &track_count);
  if (ret != PLAYER_ERROR_NONE) {
    LOG_ERROR("[VideoPlayer] player_get_track_count failed: %s",
              get_error_message(ret));
  ...
  ret = player_get_current_track(player_, PLAYER_STREAM_TYPE_AUDIO,
                                 &current_track_index);
  if (ret != PLAYER_ERROR_NONE) {
    LOG_ERROR("[VideoPlayer] player_get_current_track failed: %s",
              get_error_message(ret));
    return audio_track_result;
  }
  ...
  for (int i = 0; i < track_count; i++) {
    ...
    ret = player_get_track_language_code(player_, PLAYER_STREAM_TYPE_AUDIO, i,
                                         &language_code);
    if (ret != PLAYER_ERROR_NONE) {
      LOG_ERROR(
          "[VideoPlayer] player_get_track_language_code failed for track %d: "
          "%s",
    ...
    track_info[flutter::EncodableValue("id")] =
        flutter::EncodableValue(std::to_string(i));
    track_info[flutter::EncodableValue("label")] =
        flutter::EncodableValue(label);
    track_info[flutter::EncodableValue("language")] =
        flutter::EncodableValue(language);
    track_info[flutter::EncodableValue("isSelected")] =
        flutter::EncodableValue(i == current_track_index);

    audio_track_result.push_back(flutter::EncodableValue(track_info));

    LOG_INFO(
        "[VideoPlayer] TrackID %d - label: %s, language: %s, isSelected: %d", i,
        label.c_str(), language.c_str(), i == current_track_index);
  }
...
void VideoPlayer::selectAudioTrack(std::string track_id) {
  ...
  if (state != PLAYER_STATE_READY && state != PLAYER_STATE_PLAYING &&
    ...
    LOG_ERROR(
        "[VideoPlayer] Player is not in a valid state for selecting audio "
        "track: %s",
        StateToString(state).c_str());
    throw VideoPlayerError("Invalid player state",
                           "Player must be in READY, PLAYING or PAUSED state "
                           "to select audio track");
  }
  ...
  try {
    ...
    LOG_ERROR(
        "[VideoPlayer] Failed to convert track id to integer: %s, error: %s",
        track_id.c_str(), e.what());
    throw VideoPlayerError("Invalid trackId",
                           "trackId must be a valid integer string");
  }
  ...
  if (ret != PLAYER_ERROR_NONE) {
    LOG_ERROR("[VideoPlayer] player_get_track_count failed: %s",
              get_error_message(ret));
    throw VideoPlayerError("player_get_track_count failed",
                           get_error_message(ret));
  }
  ...
  if (ret != PLAYER_ERROR_NONE) {
    LOG_ERROR("[VideoPlayer] player_select_track failed: %s",
              get_error_message(ret));
    throw VideoPlayerError("player_select_track failed",
                           get_error_message(ret));
  }
