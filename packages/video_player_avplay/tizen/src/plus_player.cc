...
void PlusPlayer::OnSubtitleData(char *data, const int size,
                                ...
                                void *user_data) {
  LOG_INFO("[PlusPlayer] Subtitle updated, duration: %llu, type: %d", duration, type);
  ...
  for (auto attr = attrs->begin(); attr != attrs->end(); attr++) {
    LOG_DEBUG("[PlusPlayer] SubtitleAttr update: attrType: %d, start: %u, end: %u.",
    ...
    switch (attr->type) {
      ...
      case plusplayer::kSubAttrWebvttCuePosition: {
        ...
        if (attr->type == plusplayer::kSubAttrRegionWidth && type == plusplayer::SubtitleType::kPicture) {
          picture_width = (double)value_float;
        }
        if (attr->type == plusplayer::kSubAttrRegionHeight && type == plusplayer::SubtitleType::kPicture) {
  ...
  if (type == plusplayer::SubtitleType::kPicture) {
    LOG_INFO("[PlusPlayer] Subtitle is a picture: size: %d, width: %f, height: %f",
             size, picture_width, picture_height);

    int subtitle_mem_length = 0;
    int channels = size / (picture_width * picture_height);
    unsigned char *subtitle_png = stbi_write_png_to_mem((const unsigned char *)data, 0, picture_width, picture_height, channels, &subtitle_mem_length);

    if (subtitle_png && subtitle_mem_length > 0) {
      std::vector<uint8_t> picture_data(subtitle_png, subtitle_png + subtitle_mem_length);
      self->SendSubtitleUpdate(duration, std::string(), attributes_list, picture_data, picture_width, picture_height);
    ...
    LOG_INFO("[PlusPlayer] Subtitle is text: duration: %llu, text: %s, type: %d", duration,
             data, type);
